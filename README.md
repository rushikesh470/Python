...
Project created by:
Ghotekar Rushikesh Babasaheb Student of Amrutvahini College of Engineering, T.E. Computer Engineering Department (under Zensar Python and SQL Training)
...
Setup Instructions
Database Setup
Create Tables: Use the provided SQL scripts to create the tables (Alumni, Contributions, Events, Event_Management, Logs) in your database.

Insert Sample Data: Populate the tables with sample data using the INSERT INTO statements provided.

Create Sequence: Ensure that the sequence Event_Management_SEQ is created to generate unique IDs for the Event_Management table.

Create Triggers: Set up triggers to automatically log changes made to alumni records (e.g., when an alumni's information is updated).

Create Procedures: Create stored procedures such as manage_event_participation to handle event registration and alumni feedback.
